...
class FakePresenter:
    ...
    Every method answers its completion immediately, on the same call
    stack: since issue #69 a real `WorkspacePresenter` cannot do that (its
    dialogs are QML items now, asynchronous by construction), but
    `ApplicationWorkflow`'s continuation chain settles synchronously
    whenever everything it asks does, which is what makes `_result` below
    able to read an outcome without an event loop.
    """
    ...
    def choose_analysis_destination(
        ...
        on_result(self.analysis_destination)
    ...
    def choose_replacement_media(
        ...
        on_result(self.replacement_media)
    ...
    def confirm_source_video_replacement(
        ...
        on_result(self.replacement_confirmed)
...
class DeferredSavePresenter(FakePresenter):
    ...
    def ask_unsaved_changes(
        ...
        self.unsaved_prompts += 1
        self._unsaved_answer = on_result
    ...
    def answer_destination(self, destination: str | None) -> None:
...
def _result(call: Callable[[Callable[[object], None]], None]) -> object:
    """Run a continuation-based `ApplicationWorkflow` command and read its outcome.

    Every method a dialog can be behind — `may_replace_analysis`,
    `new_analysis`, `open_analysis`, `open_analysis_file`, `save`,
    `save_as`, `add_source_video`, `relink_source_video`,
    `relink_source_video_file`, `open_dropped_file` and
    `recover_if_available` — takes a completion instead of returning a
    value now (issue #69: the dialog behind it may be asynchronous QML).
    `FakePresenter` still answers every question immediately, so the
    command still settles before this returns; call it as
    ``_result(lambda done: workflow.new_analysis(done))``.
    """
...
def test_new_analysis_replaces_the_current_one_after_discarding(
    ...
    assert _result(lambda done: workflow.new_analysis(done)) is True
...
def test_cancelling_the_unsaved_prompt_keeps_the_current_analysis(
    ...
    assert _result(lambda done: workflow.new_analysis(done)) is False
...
def test_saving_an_unsaved_analysis_asks_for_a_destination_once(
    ...
    assert _result(lambda done: workflow.save(done)) is True
...
def test_save_as_always_asks_for_a_destination(
    ...
    assert _result(lambda done: workflow.save_as(done)) is True
...
def test_a_cancelled_destination_leaves_the_analysis_dirty(
    ...
    assert _result(lambda done: workflow.save(done)) is False
...
def test_replacing_after_an_async_save_waits_for_save_as_to_settle(
    recovery_store: RecoverySnapshotStore, tmp_path: Path
) -> None:
    """`may_replace_analysis` completes only after ask → save → Save As."""
    ...
    workflow.analysis.set_title("Noch nicht gespeichert")
    outcomes: list[bool] = []

    workflow.new_analysis(outcomes.append)
    assert presenter.unsaved_prompts == 1
    assert outcomes == []
    assert workflow.analysis.title == "Noch nicht gespeichert"

    presenter.answer_unsaved_changes(UnsavedChangesChoice.SAVE)
    assert presenter.suggested_names == ["Noch nicht gespeichert.analysis"]
    assert outcomes == []
    assert workflow.document.dirty is True

    destination = tmp_path / "gegen-kiel.analysis"
...
def test_opening_an_analysis_replaces_the_current_one(
    ...
    assert _result(lambda done: workflow.open_analysis(done)) is True
...
def test_unsaved_changes_are_settled_before_a_file_is_chosen(
    ...
    def record_prompt(on_result: Callable[[UnsavedChangesChoice], None]) -> None:
        asked.append("unsaved")
        on_result(UnsavedChangesChoice.CANCEL)

    def record_chooser(on_result: Callable[[str | None], None]) -> None:
        asked.append("chooser")
        on_result(presenter.analysis_to_open)
    ...
    assert _result(lambda done: workflow.open_analysis(done)) is False
...
def test_a_failed_load_leaves_the_open_analysis_untouched(
    ...
    assert _result(lambda done: workflow.open_analysis_file(broken, done)) is False
...
def test_a_rejected_load_leaves_the_open_analysis_untouched(
    ...
    assert _result(lambda done: workflow.open_analysis_file(_saved_analysis_file(tmp_path), done)) is False
...
def test_a_missing_analysis_file_is_reported(
    ...
    assert _result(lambda done: workflow.open_analysis_file(tmp_path / "absent.analysis", done)) is False
    assert len(presenter.failures) == 1
...
def test_adding_a_video_through_the_dialog_uses_the_chosen_file(
    ...
    added = _result(lambda done: workflow.add_source_video(done))
...
def test_cancelling_the_video_dialog_adds_nothing(
    ...
    assert _result(lambda done: workflow.add_source_video(done)) is None
    assert workflow.analysis.source_videos == ()
...
def test_a_dropped_video_is_added_to_the_current_analysis(
    ...
    assert _result(lambda done: workflow.open_dropped_file(_video(tmp_path, "second-half.MOV"), done)) is True
...
def test_a_dropped_analysis_file_replaces_the_current_analysis(
    ...
    assert _result(lambda done: workflow.open_dropped_file(_saved_analysis_file(tmp_path), done)) is True
...
def test_an_unsupported_dropped_file_is_ignored(
    ...
    assert _result(lambda done: workflow.open_dropped_file(note, done)) is False
    assert workflow.analysis.source_videos == ()
...
def test_closing_a_clean_analysis_is_allowed_without_asking(
    ...
    assert _result(lambda done: workflow.may_replace_analysis(done)) is True
    assert presenter.unsaved_prompts == 0
...
def test_closing_a_dirty_analysis_asks_and_can_be_refused(
    ...
    assert _result(lambda done: workflow.may_replace_analysis(done)) is False
    assert workflow.document.dirty is True
...
def test_a_verified_match_relinks_without_asking_for_confirmation(
    ...
    relinked = _result(lambda done: workflow.relink_source_video_file(source_video.id, replacement_path, done))
...
def test_a_mismatched_replacement_requires_explicit_confirmation(
    ...
    relinked = _result(lambda done: workflow.relink_source_video_file(source_video.id, unrelated_path, done))
...
def test_relinking_an_unknown_source_video_is_reported_rather_than_raised(
    ...
    """
    assert _result(lambda done: workflow.relink_source_video(uuid4(), done)) is False
    assert len(presenter.failures) == 1
...
def test_a_cancelled_relink_dialog_leaves_the_source_video_unavailable(
    ...
    relinked = _result(lambda done: workflow.relink_source_video(source_video.id, done))
...
def test_a_failed_probe_during_relink_leaves_the_source_video_unavailable(
    ...
    relinked = _result(lambda done: workflow.relink_source_video_file(source_video.id, unreadable_path, done))
...
def test_no_stored_snapshot_offers_nothing(
    ...
    assert _result(lambda done: workflow.recover_if_available(done)) is False
...
def test_an_accepted_recovery_offer_replaces_the_analysis_and_stays_dirty(
    ...
    assert _result(lambda done: workflow.recover_if_available(done)) is True
