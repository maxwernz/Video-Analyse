...
class DocumentCommands(Protocol):
    ...
    Every command here used to report success or failure by returning a
    `bool`; none of the two menu bars this module builds ever read it
    (`_command` and `command_runners` both discard what `run()` returns).
    Issue #69 made the return value actively wrong rather than merely
    unused — each of these can now end up waiting on a QML dialog, so
    "did it succeed" is not known by the time the call that triggered it
    returns — which is why they report nothing at all now.
    """

    def newAnalysis(self) -> None: ...

    def openAnalysis(self) -> None: ...

    def saveAnalysis(self) -> None: ...

    def saveAnalysisAs(self) -> None: ...

    def addSourceVideo(self) -> None: ...
